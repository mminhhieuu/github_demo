...
if (isset($_POST['thempb'])) {
    ...
    // Data validation
    if (!empty($ma_pb) && !empty($phong_ban) && !empty($email) && !empty($ghi_chu)) {
        $sql = "INSERT INTO phongban(`ma_pb`, `phong_ban`, `email`, `ghi_chu`)
            VALUES('$ma_pb', '$phong_ban', '$email', '$ghi_chu')";
        if ($conn->query($sql) === true) {
            echo "Lưu dữ liệu thành công";
            header("Location:../WEB_C9/trangchu.php");
        } else {
